...
{
    public class PCFDebugger : MonoBehaviour
    {
        ...
        public GameObject AnchorObjectForPcfId(string pcfId)
        {
            foreach (MagicLeap.XR.XRKit.MLXRApi.MLXrPCFId pcf in anchorGameObjects.Keys)
            {
                string checkPcfId = pcf.ToString();
                if (checkPcfId.Equals(pcfId))
                {
                    return anchorGameObjects[pcf].gameObject;
                }
            }
            return null;
        }

        public static PCFDebugger Instance;
        private void Awake()
        {
            if (Instance == null)
            {
